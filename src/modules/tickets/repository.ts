...
export function updateCategory(id: number, patch: Partial<Pick<TicketCategory, 'welcome_message' | 'label' | 'button_text' | 'emoji' | 'color' | 'support_role_id'>>): void {
  // SECURITY FIX: explicit allowlist prevents SQL injection via Object.keys()
  const ALLOWED: ReadonlySet<string> = new Set(['welcome_message', 'label', 'button_text', 'emoji', 'color', 'support_role_id']);
