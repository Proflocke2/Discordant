...
export async function handleSecurityMessage(msg: Message): Promise<boolean> {
  ...
  if (!violated && cfg.feat_antiphing && PHISHING_PATTERNS.some(p => p.test(msg.content))) {
    violated = true;
  ...
  if (!violated && cfg.feat_linkfilter && INVITE_PATTERN.test(msg.content)) {
    violated = true;
    await msg.delete().catch(() => {});
  ...
  // Mass-ping guard
  if (!violated && cfg.feat_masspinggard) {
  ...
  // Anti-caps
  if (!violated && cfg.feat_anticaps && msg.content.length >= 10) {
