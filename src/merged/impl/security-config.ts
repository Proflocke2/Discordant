import {
  ...
  MessageFlags, ChannelType, TextChannel,
} from 'discord.js';
...
function buildLogModal(t: (k: string) => string): ModalBuilder {
  return new ModalBuilder()
    .setCustomId('sec:logchannel:modal')
    .setTitle(t('sec.btn_log'))
    .addComponents(
      new ActionRowBuilder<TextInputBuilder>().addComponents(
        new TextInputBuilder().setCustomId('channel_id').setLabel(t('sec.modal_channel_id_label')).setStyle(TextInputStyle.Short).setPlaceholder('123456789012345678').setRequired(true).setMinLength(10).setMaxLength(25)),
    );
}
...
export default {
  ...
  async execute(ix: ChatInputCommandInteraction) {
    ...
    collector.on('collect', async (interaction) => {
      ...
      if (id === 'sec:logchannel') { await (interaction as ButtonInteraction).showModal(buildLogModal(ft)); return; }
    ...
    const awaitModal = async () => {
      try {
        const modal = await ix.awaitModalSubmit({
          filter: (i) => i.user.id === ix.user.id && (i.customId === 'sec:thresholds:modal' || i.customId === 'sec:logchannel:modal'),
          time: 5 * 60 * 1000,
        ...
        if (modal.customId === 'sec:logchannel:modal') {
          const rawId = modal.fields.getTextInputValue('channel_id').trim().replace(/[<#>]/g, '');
          const ch    = ix.guild!.channels.cache.get(rawId) as TextChannel | undefined;
          if (!ch || ch.type !== ChannelType.GuildText) {
            await modal.reply({ embeds: [error(mt('sec.channel_not_found'), mt('sec.channel_not_text', { id: rawId }))], flags: MessageFlags.Ephemeral });
          } else {
            updateSecurityConfig(gid, { log_channel_id: rawId });
            const upd = getSecurityConfig(gid);
            await modal.deferUpdate();
            await ix.editReply({ embeds: [buildStatusEmbed(upd, gid, mt)], components: buildMainButtons(upd, mt) });
          }
