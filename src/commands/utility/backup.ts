...
export default {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand(s => s.setName('auto-enable').setDescription('Turn on automatic backups (daily or weekly)')
      ...
      .addChannelOption(o => o.setName('channel').setDescription('Channel to post the backup in (required if delivery = channel)')
  ...
  async execute(ix: ChatInputCommandInteraction) {
    ...
    if (sub === 'auto-enable') {
      const interval = ix.options.getString('interval', true) as 'daily' | 'weekly';
      ...
      setGuildValue(gid, 'backup_auto_interval', interval);
      ...
      return ix.editReply({
        embeds: [success(tGuild(gid, 'backup.auto.enabled', { interval, delivery, target }))],
      });
    }
    ...
    if (sub === 'auto-status') {
      ...
      return ix.editReply({
        embeds: [info(tGuild(gid, 'backup.auto.status_title'), tGuild(gid, 'backup.auto.status_on', {
          interval: cfg.interval, delivery: cfg.delivery, target, last: cfg.lastRun ?? 'never yet',
        }))],
      });
    }
  },
